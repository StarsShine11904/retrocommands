...
import static com.periut.retrocommands.command.argument.EntitySummonArgumentType.entitySummon;
import static com.periut.retrocommands.command.argument.EntitySummonArgumentType.getEntitySummon;
...
import static com.periut.retrocommands.command.argument.ItemArgumentType.getItem;
import static com.periut.retrocommands.command.argument.ItemArgumentType.item;
    ...
    private static int give(final CommandContext<RetroCommandSource> context, final int count, final String entity) throws CommandSyntaxException {
        ...
        for (final PlayerEntity target : targets) {
            ...
            while (remaining > 0 && !full) {
                ...
                if (CommandUtil.give(source, target, stack)) {
                    remaining -= stackSize;
                } else {
                    full = true;
                }
        ...
        if (targets.size() == 1) {
            source.sendFeedback(Text.literal("Gave " + count + " ").append(description).append(" to " + targets.get(0).name));
        } else {
