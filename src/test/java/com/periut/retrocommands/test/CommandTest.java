...
import com.periut.retrocommands.command.argument.CoordinateArgument;
import com.periut.retrocommands.command.argument.DefaultPosArgument;
...
import com.periut.retrocommands.command.argument.VanillaIds;
...
public final class CommandTest {
    ...
    private static void targetSuggestions() {
        ...
        dispatcher.register(LiteralArgumentBuilder.<RetroCommandSource>literal("tp")
            .then(RequiredArgumentBuilder.<RetroCommandSource, com.periut.retrocommands.command.selector.EntitySelector>argument(
                "targets", com.periut.retrocommands.command.argument.EntityArgumentType.players())
                .executes(context -> 1)));
    ...
    private static void displayNames() {
        ...
        Tests.eq("spelling out an identifier", "Iron Ingot", com.periut.retrocommands.command.argument.ItemNames.spellOut("iron_ingot"));
        Tests.eq("a single word", "Apple", com.periut.retrocommands.command.argument.ItemNames.spellOut("apple"));
        ...
        Tests.eq("a block", "Stone", com.periut.retrocommands.command.argument.ItemNames.displayName(1, 0));
        Tests.eq("an item", "Apple", com.periut.retrocommands.command.argument.ItemNames.displayName(260, 0));
        Tests.eq("a two-word item", "Iron Ingot", com.periut.retrocommands.command.argument.ItemNames.displayName(265, 0));

        // Beta's lang file has one entry for all sixteen wools, so only the mod can name this.
        Tests.eq("a subtype names itself", "Red Wool", com.periut.retrocommands.command.argument.ItemNames.displayName(35, 14));
        Tests.eq("subtype zero uses the plain name", "Wool", com.periut.retrocommands.command.argument.ItemNames.displayName(35, 0));
        Tests.eq("a dye subtype", "Bone Meal", com.periut.retrocommands.command.argument.ItemNames.displayName(351, 15));
        ...
        for (final String name : VanillaIds.names()) {
            ...
            final String display = com.periut.retrocommands.command.argument.ItemNames.displayName(entry.id(), 0);
        ...
        Tests.eq("a lang key resolves when the file is present", "Raw Porkchop",
            com.periut.retrocommands.text.Translations.find("item.porkchopRaw.name"));
        Tests.eq("the lang file has one name for every wool", "Wool",
            com.periut.retrocommands.text.Translations.find("tile.cloth.name"));

        Tests.check("an unknown key is reported as unknown",
            com.periut.retrocommands.text.Translations.find("retrocommands.no.such.key") == null);
        Tests.eq("an unknown key falls back", "fallback",
            com.periut.retrocommands.text.Translations.get("retrocommands.no.such.key", "fallback"));
    }
    ...
    private static List<String> suggestIdentifiers(final List<String> ids, final String typed) {
        ...
        return com.periut.retrocommands.command.SuggestionHelper.suggestIdentifiers(ids, builder).join()
    ...
    private static void positionSuggestions() {
        ...
        final CommandDispatcher<RetroCommandSource> dispatcher = new CommandDispatcher<>();
        dispatcher.register(LiteralArgumentBuilder.<RetroCommandSource>literal("tp")
            .then(RequiredArgumentBuilder.<RetroCommandSource, com.periut.retrocommands.command.argument.PosArgument>argument(
                "location", com.periut.retrocommands.command.argument.Vec3ArgumentType.vec3())
                .executes(context -> 1)));
    ...
    private static RetroCommandSource sourceWithPlayers(final String... names) {
        ...
        }, null, null, com.periut.retrocommands.command.Position.ORIGIN, 0.0f, 0.0f, "test",
